...
pub enum Backend {
    /// Pick whichever backend can decode this file correctly: the builtin
    /// path when it is safe (H.264, CABAC entropy, not BT.2020), ffmpeg
    /// otherwise. Never CAVLC/`Unknown`/BT.2020 into the builtin path.
    Auto,
    /// The builtin path, or an error naming the reason when it is unsafe
...
/// [`crate::video::ffmpeg::FfmpegSource::probe`]) -- the builtin backend has
/// no equivalent of its own (`BuiltinVideoSource::open_path` takes no such
...
pub fn open_video(
    ...
    match backend {
        ...
        Backend::Builtin => {
            open_builtin_checked(path).map(|s| Box::new(s) as Box<dyn FrameSource>)
        }
        // PREFER ffmpeg when it is ALREADY present. It decodes far more than
        // the builtin path -- HEVC, 10-bit, VP9, AV1, and every container --
        // while the builtin path is H.264/CABAC only and refuses CAVLC,
        // `Unknown` entropy and BT.2020 outright because it decodes them to
        // wrong pixels silently. Preferring it removes that whole class of
        // failure for anyone who has ffmpeg.
        //
        // Only "already available" counts: this asks `ffmpeg_present` rather
        // than `ensure_ffmpeg`, so `Auto` can still never trigger a download.
        // Acquiring ffmpeg stays an explicit `--backend ffmpeg` decision.
        Backend::Auto if ffmpeg_present() => {
            // Still fall back to builtin if ffmpeg fails, so preferring it
            // can only ever add capability, never remove it.
            open_ffmpeg(path, target, fit, filter, fps).or_else(|ffmpeg_err| {
                open_builtin_checked(path)
                    .map(|s| Box::new(s) as Box<dyn FrameSource>)
                    .map_err(|_| ffmpeg_err)
            })
        }
        Backend::Auto => match open_builtin_checked(path) {
            Ok(source) => Ok(Box::new(source)),
            // Whatever blocked the builtin path (wrong codec, CAVLC,
            ...
            // user explicitly asked for `Backend::Builtin`, which is the branch
            // above, not this one.
...
/// Whether an ffmpeg binary is already usable, without fetching one.
///
/// Native only. On wasm no ffmpeg backend exists at all (it would have to
/// spawn a subprocess), so this is always `false` and [`Backend::Auto`] goes
/// straight to the builtin decoder.
...
/// Builds a [`BuiltinVideoSource`] for `path`, refusing (with a message naming
/// the reason) anything this task's two guards say is unsafe.
///
/// Layers the BT.2020 guard on top of [`BuiltinVideoSource::open_path`]'s own
...
fn open_builtin_checked(path: &Path) -> Result<BuiltinVideoSource, String> {
    let source = BuiltinVideoSource::open_path(path)?;
    ...
    // rather than threading one through: `BuiltinVideoSource` does not expose
    // its internal demuxer, and `BuiltinFrameStream::new` already re-opens one
...
/// Every failure in this module and in `BuiltinVideoSource::open_path` is a plain
...
/// Classifies why the builtin backend failed on `path`; see [`BuiltinFailure`].
///
...
/// (see [`open_builtin_checked`], which opens a second [`Demuxer`] for exactly
/// the same reason), and the alternative would mean changing
/// `BuiltinVideoSource::open_path`'s established `Result<_, String>` signature.
/// Only ever called on a path that has ALREADY failed, so the extra read costs
/// nothing on the success path.
pub fn classify_builtin_failure(path: &Path) -> BuiltinFailure {
    if std::fs::File::open(path).is_err() {
        return BuiltinFailure::Unreadable;
    }
    if Demuxer::open(path).is_err() {
        return BuiltinFailure::Container;
    }
    // Readable, and the container parsed -- so whatever `open_builtin_checked`
    // refused, it refused *after* those two steps, which leaves exactly the
    // guards.
    BuiltinFailure::Guard
}

/// [`open_video`], but resolving [`Backend::Auto`] by *trying* the builtin
...
/// including the CABAC H.264 files the builtin backend decodes perfectly
...
///
/// Per backend:
/// - [`Backend::Builtin`]: never calls `ensure_ffmpeg`. The guards still refuse
...
/// - [`Backend::Auto`]: builtin first; `ensure_ffmpeg` only once that has
///   failed with a [`BuiltinFailure`] ffmpeg could actually help with.
pub fn open_video_ensuring(
    ...
    match backend {
        Backend::Builtin => open_video(path, Backend::Builtin, target, fit, filter, fps),
        Backend::Ffmpeg => {
            ensure_ffmpeg()?;
            open_video(path, Backend::Ffmpeg, target, fit, filter, fps)
        }
        Backend::Auto => {
            let builtin_err = match open_video(path, Backend::Builtin, target, fit, filter, fps) {
                // The builtin backend handled it, so ffmpeg was never
            ...
            if classify_builtin_failure(path) == BuiltinFailure::Unreadable {
...
/// The refusal [`open_builtin_checked`] returns for a BT.2020-tagged stream.
...
/// the builtin guards refuse has no correct decode path here. This errors
...
fn open_ffmpeg(
    ...
    Err(format!(
        "{}: no ffmpeg decode backend is available in this build (the browser build cannot \
         spawn a subprocess), and this file is not safe for the builtin decoder either; \
...
mod tests {
    ...
    /// for the builtin backend by name.
    #[test]
    fn cavlc_never_reaches_the_builtin_decoder() {
        ...
        // Asking for builtin by name must ERROR, not decode wrongly.
        //
        ...
        let err = match open_video(&path, Backend::Builtin, None, FitMode::Contain, Filter::Lanczos, None) {
            Err(e) => e,
            Ok(_) => panic!("explicit --backend builtin must refuse a CAVLC stream"),
        };
        assert!(
    ...
    /// A CABAC stream is what the builtin decoder is correct on, so it must
    /// NOT be diverted -- otherwise the guard has quietly disabled backend B.
    #[test]
    fn cabac_still_reaches_the_builtin_decoder() {
        let Some(path) = crate::video::ffmpeg::tests::sample_clip_args(
            "cabac", &["-coder", "1"], 1, 32, 32, 5,
        ) else { return };
        let s = open_video(&path, Backend::Builtin, None, FitMode::Contain, Filter::Lanczos, None)
            .expect("explicit --backend builtin must accept a CABAC stream");
        let mut st = s.open().expect("open");
    ...
    fn bt2020_never_reaches_the_builtin_decoder() {
        ...
        let err = match open_video(&path, Backend::Builtin, None, FitMode::Contain, Filter::Lanczos, None) {
            Err(e) => e,
            Ok(_) => panic!("explicit --backend builtin must refuse a BT.2020-tagged stream"),
        };
        assert!(
    ...
    /// builtin attempt has failed.
    #[test]
    fn auto_consults_ffmpeg_only_after_the_builtin_path_refuses() {
        let Some(path) = crate::video::ffmpeg::tests::sample_clip_args(
    ...
    /// ffmpeg available is an ERROR, never a silently wrong builtin decode.
    ...
    fn a_guarded_stream_with_no_ffmpeg_errors_naming_both_reasons() {
        ...
        assert!(
            err.to_uppercase().contains("CAVLC"),
            "the builtin refusal must survive into the message: {err}"
        );
        assert!(
    ...
    fn combined_refusal_gives_coherent_advice_not_contradictory_suggestions() {
        ...
        let mut simulated_ffmpeg_refusal = || {
            Err("ffmpeg was not found and downloading it was declined. Install ffmpeg \
                 yourself and make sure it is on PATH, or run with --backend builtin to use the \
                 builtin decode path instead."
                .to_string())
        };
        let err = match open_video_ensuring(
            &path, Backend::Auto, None, FitMode::Contain, Filter::Lanczos, None,
        ...
        assert!(
            ...
            "the builtin half's own suggestion must still be visible: {err}"
        );
        assert!(
            err.contains("run with --backend builtin"),
    ...
    /// `--backend builtin` must never consult ffmpeg, on any file. The closure
    /// panics if called, so a regression here is unmissable.
    #[test]
    fn explicit_builtin_never_consults_ffmpeg() {
        let Some(path) = crate::video::ffmpeg::tests::sample_clip_args(
            "explicit_builtin", &["-coder", "1"], 1, 24, 24, 5,
        ) else { return };
        let source = match open_video_ensuring(
            &path, Backend::Builtin, None, FitMode::Contain, Filter::Lanczos, None,
            &mut || panic!("--backend builtin must never consult ffmpeg"),
        ) {
            Ok(s) => s,
            Err(e) => panic!("explicit builtin must accept a CABAC stream: {e}"),
        };
    ...
    #[test]
    fn explicit_builtin_refuses_a_guarded_stream_without_consulting_ffmpeg() {
        let Some(path) = crate::video::ffmpeg::tests::sample_clip_args(
            "explicit_builtin_cavlc", &["-coder", "0", "-bf", "0"], 1, 24, 24, 5,
        ) else { return };
        let err = match open_video_ensuring(
            &path, Backend::Builtin, None, FitMode::Contain, Filter::Lanczos, None,
            &mut || panic!("--backend builtin must never consult ffmpeg"),
            ...
            Err(e) => e,
            Ok(_) => panic!("--backend builtin must refuse a CAVLC stream"),
        };
    ...
    #[test]
    fn a_builtin_failure_is_classified_by_what_actually_went_wrong() {
        let missing = std::env::temp_dir()
            .join(format!("h2b_absent_{}_{}.mp4", std::process::id(), line!()));
        let _ = std::fs::remove_file(&missing);
        assert_eq!(classify_builtin_failure(&missing), BuiltinFailure::Unreadable);
        ...
        assert_eq!(
            classify_builtin_failure(&garbage),
            BuiltinFailure::Container,
        ...
        assert_eq!(
            classify_builtin_failure(&cavlc),
            BuiltinFailure::Guard,
    ...
    /// guard has quietly disabled the builtin backend for every tagged
    ...
    fn bt709_is_not_mistaken_for_bt2020() {
        ...
        let s = open_video(&path, Backend::Builtin, None, FitMode::Contain, Filter::Lanczos, None)
            .expect("explicit --backend builtin must accept a BT.709-tagged CABAC stream");
        let mut st = s.open().expect("open");
        assert!(st.next().expect("next").is_some());
        let _ = std::fs::remove_file(&path);
    }
