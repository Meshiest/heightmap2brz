...
/// Report a fatal CLI error and exit NON-ZERO -- `error!`'s formatting, [`fail`]'s
/// exit code.
///
/// **Every failure path in this file goes through this or through [`fail`].**
/// `return error!(...)` -- what all but seven of them used to be -- only PRINTS:
/// `main` then returns `()` and the process exits 0, so
/// `heightmap song.mp3 --audio-mode voice --max-voices 0 -o out.brz && upload out.brz`
/// refuses the render, writes nothing, and runs the upload anyway. Worse, the
/// SAME error exited differently depending on which branch raised it (an unknown
/// `--font` exited 1 under `--anim-mode text` and 0 under `--text`), so a script
/// could not even learn the rule by trying it.
///
/// A macro rather than a bare `fail(format!(...))` at every site purely so the
/// call reads like the `error!` it replaces and cannot lose its arguments in the
/// conversion.
#[cfg(not(target_arch = "wasm32"))]
macro_rules! fail {
    ($($arg:tt)+) => {
...
fn cli() -> clap::App<'static, 'static> {
    clap_app!(heightmap =>
        ...
        (@arg audiomaxvoices: --("max-voices") +takes_value "Audio: in --audio-mode bank (Pitch-Per-Speaker), an UPPER BOUND on bands sounding at once (default 12); in --audio-mode voice (Pitch Switching), the NUMBER OF SPEAKERS BUILT, every one of them sounding most frames (32 simultaneous emitters are verified to sound; above that is untested). In Pitch-Per-Speaker this is capped by --peak-gate, not by itself: raising it past the ~12-24 candidates the gate leaves changes nothing at all. 0 = every band (Pitch-Per-Speaker only; an error in Pitch Switching)")
        ...
        (@arg noloop: --("no-loop") "Play through ONCE and stop on the last frame, instead of looping forever (the default). Video and audio alike -- both drive the same clock. The Timer's Limit becomes (frames - 0.5) / fps, i.e. it expires halfway through the final frame's slot so the frame index lands on frames-1; the clip's exact duration would instead land on frames % frames = 0 and snap the render back to its FIRST frame. Costs nothing -- identical gates, wires and bricks either way. Inert with --external-clock, which builds no timer at all")
...
fn main() {
    ...
    // (`pack::Packer::linearize`) and nothing else, so on any other path it is a
    // flag that parses and does nothing. Named out loud rather than dropped,
    // exactly as the subtitle flags above are, and pointing at the equivalent
    // that path does have.
    ...
    if matches.is_present("animmode") {
        ...
        // Colour-array mode converts sRGB -> linear unconditionally: its array
        // elements are documented as linear 0-1 and nothing downstream of the
        // renderer transforms them, unlike the hex path where `MakeColorHex`
        // does its own conversion and `--srgb-to-linear` is a real question.
        // Said out loud rather than silently ignored -- a user who passed the
        // flag and saw no difference would reasonably think it was broken.
        ...
        // The third and last place the flag can be inert, named for the same
        // reason: text mode writes `<color=\"RRGGBB\">` tags straight out of the
        // frame and never consults it.
        ...
        if heightmap_files.len() == 1 && is_video_path(&heightmap_files[0]) {
            ...
            // The SAME builder the image/sequence path below uses -- see
            // `anim_options`, and `--srgb-to-linear` for what the two copies
            // this replaces disagreed about.
        ...
        // how many arrays the frames spill across. THE SAME builder the video
        // path above uses: this branch's own copy of it omitted
        // `srgb_to_linear` entirely, so `--srgb-to-linear` on a GIF/APNG/WebP
        // or a frame sequence parsed, said nothing, and rendered as though it
...
/// Print the pre-render cost readout for whichever mode was chosen.
///
/// The three modes do not have the same fields worth reporting, so this is
/// not one format string with different numbers in it: hex mode's chunk and
/// character counts are meaningless in colour-array mode (which tiles nothing
/// and writes no strings), and colour-array mode's host-memory figure is the
/// number that actually decides whether a long render is feasible -- 16 bytes
/// per pixel per frame against hex mode's 6. Printing a "0 chunk(s)" line for
/// colour-array mode would read as a bug rather than as the true answer.
///
/// `width`, `height` and `char_repeat` are only read in text mode, where the
/// per-band character figure is a BOUND computed from the geometry rather than
/// a field of `Cost` -- see the `AnimMode::Text` arm.
#[cfg(not(target_arch = "wasm32"))]
fn log_cost(mode: AnimMode, cost: &cost::Cost, width: u32, height: u32, char_repeat: usize) {
    ...
    match mode {
        ...
        // **A BOUND, and labelled as one.** `Cost::chars` is 0 in text mode by
        // design (see `cost::estimate_text`): unlike hex mode's fixed per-pixel
        // stride, a text render's length depends on how the clip's colours
        // actually run, which nothing can know before the render. Printing that
        // 0 here would read as a measurement of an empty save, and inventing a
        // guess would be worse -- so what is printed instead is the closed-form
        // worst case the band layout was derived from, said out loud to be a
        // ceiling. Real renders come in far under it (`--colors` is the lever).
        AnimMode::Text => {
            ...
            match text_layout::plan_bands(width as usize, height as usize, repeat) {
                Ok(plan) if !plan.is_empty() => {
                    ...
                    // No caveat line needed: `cost::estimate_text` now takes
                    // `char_repeat`, so the gate/brick estimate above and the
                    // band count here are computed from the same layout and
                    // cannot disagree. They did once, when the estimate
                    // hard-coded 2 -- a single-glyph font bands 192x108 36 ways
                    // while that line still claimed 54.
                }
                // A layout this geometry cannot support, or a zero-height clip.
                // Not reported here: the render starts immediately below and
                // fails with `plan_bands`' own message, which names the width,
                // the limit and the two flags that fix it.
                _ => {}
            }
        }
        AnimMode::Brick(AnimEncoding::ColorArray) => {
            // Frames can be 0 here when the source could not report its length
            // ahead of decode (see the `frame_count_hint` warning above), in
            // which case both of these are 0 too -- honest rather than guessed.
...
///
/// `error!` alone only prints: `main` returns `()` immediately afterwards, so
/// the process still exits 0 and a script that checks the status code sees a
/// successful run that produced no save.
///
/// **This is now the only way this file reports a failure.** Callers that want
/// `error!`-style formatting use the [`fail!`] macro, which is this function
/// with a `format!` in front of it; `tests/cli_exit_codes.rs` drives one
/// representative failure out of every branch of `main` and asserts a non-zero
/// status, because a site that silently loses its exit code again would
/// otherwise look exactly like one that never had a problem.
#[cfg(not(target_arch = "wasm32"))]
...
/// passed and the source's own dimensions stand.
///
/// ONE parser for both `--anim-mode` branches, [`text_options`]' reason again:
/// the video branch parsed these cleanly while the image/sequence branch
/// `.expect(...)`ed them, so `--width abc` was a CLI error on an mp4 and a panic
/// trace on a GIF -- decided by `is_video_path`, which is not something the user
/// typed.
///
/// **Zero is refused, not accepted.** `--width 0 --height 0` used to render a
/// "Done!" save containing 0 pixels in brick mode (while text mode refused the
/// identical input, and the GUI's sliders start at 1). A screen with no pixels
/// on it is not a smaller render, it is a save with nothing in it.
#[cfg(not(target_arch = "wasm32"))]
fn target_size(
    matches: &clap::ArgMatches,
    native_w: u32,
...
/// The `AnimOptions` both `--anim-mode` branches share: the display flags
/// (`--brick-style`, `--pixel-extent`, `--glow`, `--srgb-to-linear`), the clock
/// flags (`--external-clock`, `--no-loop`), and the pieces the caller has
/// already parsed for its own use.
///
/// ONE builder rather than one per branch, for [`text_options`]' reason and with
/// a measured example of it: the video branch's copy set `srgb_to_linear` and
/// the image/sequence copy did not, so `--srgb-to-linear` worked on an mp4 and
/// silently did nothing on a GIF, an APNG, a WebP or a frame sequence -- the
/// same flag, the same mode, the same encoder reading it
/// (`pack::Packer::linearize`), and no message either way.
#[cfg(not(target_arch = "wasm32"))]
fn anim_options(
    matches: &clap::ArgMatches,
    colors: usize,
    ...
    Ok(AnimOptions {
        ...
        colors,
        // BOTH fields, from the ONE flag. They are read by different code: the
        // renderer culls against `alpha_threshold`, while the median-cut palette
        // is built against `text.alpha_threshold`, because that is the encoder's
        // own visibility rule (`text::encode_row` skips pixels under it
        // entirely). Setting only one would build the palette against a
        // different notion of which pixels are visible than the one drawn.
...
/// The `TextOptions` that every text-rendering path shares, built from the
/// flags that describe a glyph: `--font`, `--fill-char`, `--empty-char`,
/// `--char-repeat`, `--alpha-threshold`, `--line-height-world`,
/// `--braille`/`--blocks`, `--luma-threshold`, `--invert` and `--material`.
///
/// ONE builder rather than one per path. `--text` (a still image) and
/// `--anim-mode text` (a clip) need exactly the same options, and two parsers
/// over one set of flags drift apart silently -- each looks correct on its own,
/// and the only symptom is that the same flags render two different pictures.
///
/// Numeric values go through [`parse_arg`], so a mistyped `--char-repeat two`
/// is a CLI error naming the flag rather than the panic trace the inline
/// `.expect(...)` calls this replaces used to produce.
#[cfg(not(target_arch = "wasm32"))]
...
/// Every `--audio-mode` numeric flag, or its default.
///
/// Returns a `Result` so one `?` per flag reports the first bad value and
/// stops, instead of five near-identical `match` blocks inline.
///
/// Range and consistency checks are left to the modules that own them --
/// `BandPlan` rejects an impossible band split, a span the emitter's pitch
/// range cannot hold, and a zero subdivision; `analyze` rejects a non-positive
/// rate. All of them name these same flags, so duplicating the checks here
...
mod tests {
    ...
    /// THE BACKWARD-COMPATIBILITY GUARANTEE for `--no-loop`: absent, playback
    /// loops, which is what every render did before the flag existed. A
    /// default that flipped would change every existing command line's output
    /// without anyone typing anything.
    ///
    /// Parsed through the real `cli()` (see `args`), so this also proves the
    /// flag is actually registered and spelled `--no-loop`.
    #[test]
    ...
    /// **`--srgb-to-linear` reaches the options on EVERY `--anim-mode` input.**
    ///
    /// It used to be set in the video branch's copy of this struct and omitted
    /// from the image/sequence branch's, so `heightmap clip.gif --anim-mode
    /// brick --srgb-to-linear` rendered exactly as though the flag had never
    /// been passed -- same encoder (`pack::Packer::linearize` is the only
    /// reader), same mode, no message either way. One builder is what makes
    /// that unrepresentable; this asserts the field it turns on.
    #[test]
